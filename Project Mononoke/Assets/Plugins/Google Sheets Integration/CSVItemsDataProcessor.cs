...
namespace Plugins.GoogleSheetsIntegration
{
    public static class CSVItemsDataProcessor
    {
        ...
        private const int DESCRIPTION_COLUMN_INDEX = 9;


        private const char CELL_SEPARATOR = ',';
        public static List<ItemData> ProcessData(string cvsRawData)
        {
            ...
            for (var i = rawDataStartIndex; i < rows.Length; i++)
            {
                ...
                data = new ItemData(id, name, weight, volume, price, durability, stackCapacity, description);
                itemsData.Add(data);
            }
            return itemsData;
